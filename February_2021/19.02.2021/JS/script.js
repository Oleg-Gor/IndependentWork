...
/* var result = '2+3= ... 25+50='.replace(/(\d+)\+(\d+)=/g, function (match, match1, match2) {
  ...
  var sum = Number(match1) + Number(match2);
...
С помощью позитивного и негативного просмотра найдите все строки по шаблону 3 буквы a, 
затем буква b и поменяйте 3 буквы a на знак '!'. 
То есть из 'aaab' нужно сделать '!b'. Показать решение.

С помощью позитивного и негативного просмотра найдите все строки по шаблону 3 буквы a, 
затем любая буква, но не b и поменяйте 3 буквы a на знак '!'. 
То есть из, к примеру, 'aaaw' нужно сделать '!w', а 'aaab' не поменяется. Показать решение. */




/* let strNegativ = 'aaab cccl ssdfsb sdfsfa sdfb';

console.log(strNegativ.replace(/(^|\s)([a-zA-Z]{3})(?!b)/g,' !')); */




/* Дана строка с целыми числами. С помощью регулярки преобразуйте строку так, 
чтобы вместо этих чисел стояли их квадраты. Показать решение.

Дана строка с целыми числами. Найдите числа, стоящие в кавычках и увеличьте их в два раза. 
Пример: из строки 2aaa'3'bbb'4' сделаем строку 2aaa'6'bbb'8'. Показать решение.

Дана строка в которой есть вставки {{текст}}. Найдите все такие вставки и поменяйте в них порядок букв 
на обратный (то есть из 'текст' нужно сделать 'тскет'). Показать решение.

Дана строка '23 + 35 ='. Числа могут быть любыми. 
Выведите на экран результат операции в виде '23 + 35 = 58'. Показать решение. */


/* let arrNumbers = "2aaa 3 {текст} bbb 4 {текст}";
let arrNumb = '1 2 3 5 8';
let strNumbers = '23 + 35 = ';

console.log(strNumbers.replace(/(\d+)\s?\+\s?(\d+)\s?=\s?/g, function(find, find1, find2){

let summ = +find1 + +find2;
...
с помощью одного только регулярного выражения. Показать решение.

С помощью test определите, что переданная строка является корректным временем 
вида '12:59', '23:41', '00:12', '00:00', '09:15'. 
Время '24.00', '25.00', '12.60', '12.93', '41.93' является некорректным. Показать решение.

С помощью test определите, что переданная строка является корректным временем 
вида '9.59 am', '12.30 pm'. Показать решение.

Удалите одной регуляркой все слова из предложения, 
содержащие две одинаковые следующие друг за другом буквы. Показать решение.

Удалите одной регуляркой все повторяющиеся слова из строки, 
например для 'dsf xxx xxx sd' должно вернуть 'dsf xxx sd'. Показать решение.

Решите предыдущую задачу с учетом того, что слово может повторяться много раз. 
Например 'dsf xxx xxx xxx xxx xxx sd' должно вернуть 'dsf xxx sd'. Показать решение. */


//console.log(/^(19\d\d|20\d\d|2100)$/.test('19500'));


/* console.log(/[0-1]?[0-9]:[0-5][0-9]|[2][0-3]:[0-5][0-9]/.test('19:63'));
console.log(/^([01]\d|2[0-3]):[0-5]\d$/.test('24:00'));

console.log(/^\d[0-2]?\.[0-5]\d\s?(am|pm)$/.test('01.00 am')); */



console.log("aca zzqzqaq hsddds".replace(/\W*\w*(\w)\1{2,}/g, " !")); 


console.log(  'dsf xxx xxx xxx sd'.replace(/\b(\w+)\b(?:\s+\1)+/g, "$1"));
